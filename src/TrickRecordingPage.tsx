...
const TrickRecordingPage: React.FC<Props> = ({ trickName, onClose }) => {
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-6 text-center">
            ...
            {status === 'confirming' && (
                <div className="flex gap-4">
                    <button onClick={handleConfirm} className="bg-green-600 text-white font-bold py-3 px-8 rounded-lg">Yes, Save</button>
                    <button onClick={handleDiscard} className="bg-red-600 text-white font-bold py-3 px-8 rounded-lg">No, Discard</button>
                </div>
            )}
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white p-4 sm:p-6 flex flex-col">
            ...
            <main className="w-full max-w-2xl mx-auto flex-grow">
                <p className="text-gray-400 text-center mb-6">Record at least 3 clean takes of the trick. The more, the better the recognition will be.</p>
                
                <div className="space-y-3">
                    {Array.from({ length: MAX_TAKES }).map((_, index) => {
                        const isRecorded = !!takes[index];
                        return (
                            <div key={index} className="bg-neutral-800 rounded-lg p-4 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    ...
                                    <span className="font-semibold">{trickName} #{index + 1}</span>
                                </div>
                                <button
                                    onClick={() => handleStart(index)}
                                    className="bg-red-600 text-white font-bold py-2 px-5 rounded-md hover:bg-red-700 transition-colors text-sm"
                                >
