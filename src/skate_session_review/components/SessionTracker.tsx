    ...
    <div className="flex flex-col items-center justify-center p-6 bg-gray-800 rounded-lg shadow-lg min-h-[520px]">
        ...
        <div className="w-full text-center">
            <div className="mb-10 p-6 bg-gray-900 rounded-xl border border-gray-700">
                <p className="text-gray-300 text-sm leading-relaxed">
                   Sensors must be initialized.<br/>
                   Tap the button to authorize.
                </p>
            </div>
            <button onClick={initSensors} className="w-full bg-blue-600 text-white font-black py-5 text-xl rounded-2xl shadow-xl active:scale-95 transition-transform">
              INITIALIZE SENSORS
            </button>
        </div>
      )}
      ...
      {status === 'armed' && (
        <div className="text-center w-full">
            ...
            <p className="text-gray-400 text-sm mb-8 leading-relaxed">
                ...
                <span className={`${firstSlapDetected ? 'text-yellow-400 scale-110 font-black' : 'text-red-500 font-bold'} inline-block transition-all underline decoration-2`}>
                   Double-tap phone
                </span> to start.
            </p>
            ...
            <button onClick={startRecording} className="w-full bg-green-500 text-gray-900 font-black py-4 text-xl rounded-xl shadow-lg active:scale-95 mb-4">
              START MANUALLY
            </button>
        </div>
      )}
      ...
      {status === 'tracking' && (
        <div className="text-center w-full">
            ...
            <div className="text-8xl font-black text-white mb-10 tabular-nums tracking-tighter">
                {Math.floor(elapsedTime / 60).toString().padStart(2, '0')}:{(elapsedTime % 60).toString().padStart(2, '0')}
            </div>
            
            <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-gray-900 p-4 rounded-2xl border border-gray-700">
                    <p className="text-[10px] text-gray-500 uppercase font-black mb-1">Rotation</p>
                    <p className="text-3xl font-black text-cyan-400 tabular-nums">{liveYaw}°</p>
                </div>
                <div className="bg-gray-900 p-4 rounded-2xl border border-gray-700">
                    <p className="text-[10px] text-gray-500 uppercase font-black mb-1">Last Force</p>
                    <p className="text-3xl font-black text-white tabular-nums">{currentG.toFixed(1)}G</p>
                </div>
            </div>
            
            <div className="flex items-center justify-center gap-2 text-xs text-gray-500 font-mono mb-8 uppercase">
                <span className="w-2 h-2 bg-green-500 rounded-full animate-ping"></span>
                {pointsRecorded} data points captured
            </div>
            
            <button onClick={stopSession} className="w-full bg-red-600 text-white font-black py-5 text-2xl rounded-2xl shadow-2xl active:scale-95 transition-all">
              STOP & SAVE
            </button>
        </div>
      )}
      ...
      {status !== 'tracking' && (
        <div className="mt-auto w-full pt-4">
            <button onClick={onBack} className="w-full bg-transparent text-gray-500 font-bold py-2 rounded-lg hover:text-white transition-all text-[10px] uppercase tracking-[0.3em]">
              Cancel
            </button>
        </div>
      )}
